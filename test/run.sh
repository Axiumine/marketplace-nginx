...
#
# Runs `nginx -t` against the real files in marketplace-nginx/, then starts nginx against stand-in backends
...
# lives inside the container, which is removed on exit. marketplace-nginx/ is mounted read-only.
#
